...
class FNN_BN(nn.Module):
    def __init__(self, layers, activation_str):
        ...
        # batch normalization apllied only in hidden layers
        self.batch_layers = nn.ModuleList(
            [nn.BatchNorm1d(self.layers[i]) for i in range(1, len(self.layers) - 1)])
    ...
    def forward(self,x):
        ...
        for i, linear in enumerate(self.linears[1:-1]):
            x = activation(self.batch_layers[i-1](linear(x)), self.activation_str)
...
class DeepONet(nn.Module):
    def __init__(self, layers_dict, activation_dict):
        ...
        if arc_b == "FNN":
            ...
            self.branch  = FNN_BN(self.layers_b, self.act_b)
            
        if arc_t == "FNN":
            self.trunk   = FNN(self.layers_t, self.act_t)
        elif arc_t == "FNN_BN":
            self.trunk  = FNN_BN(self.layers_t, self.act_t)
