...
if __name__=="__main__":
    ...
    modelname = "peano_test/model_wno_test_013"
    ...
    fig, ax = plt.subplots(3, len(idx), figsize = (18, 4))
    #fig.suptitle('Applied current (I_app)')
    ax[0][0].set(ylabel = 'I_app(t)')
    for i in range(len(idx)):
        ax[0][i].plot(x_test_unscaled, esempio_test[i])
        ax[0][i].set(xlabel = 't')
        ax[0][i].set_ylim([-0.2, 10])
        ax[0][i].grid()
    #plt.show()
    #### Approximate classical solution
    #fig, ax = plt.subplots(1, len(idx), figsize = (18, 4))
    #fig.suptitle('Numerical approximation (V_m)')
    ax[1][0].set(ylabel = 'V_m (mV)')
    for i in range(len(idx)):
        ax[1][i].plot(x_test_unscaled, sol_test[i].to('cpu'))
        ax[1][i].set(xlabel = 't')
        ax[1][i].set_ylim([-100, 35])
        ax[1][i].grid()
    #plt.show()
    ...
    #fig, ax = plt.subplots(1, len(idx), figsize = (18, 4))
    fig.suptitle(arc + ' approximation (V_m)')
    ax[2][0].set(ylabel = 'V_m (mV)')
    for i in range(len(idx)):
        ax[2][i].plot(x_test_unscaled, out_test[i])
        ax[2][i].set(xlabel = 't')
        ax[2][i].set_ylim([-100, 35])
        ax[2][i].grid()
    plt.show()